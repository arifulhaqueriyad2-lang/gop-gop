#𝘛𝘏𝘌 𝘎𝘌𝘕𝘌𝘙𝘈𝘛𝘖𝘙 𝘉𝘠 𝘚𝘛𝘈𝘙 𝘎𝘈𝘔𝘌𝘙 
#𝘛𝘌𝘓𝘌𝘎𝘙𝘈𝘔 𝘐𝘋 : @𝘚𝘛𝘈𝘙_𝘎𝘔𝘙
#𝘊𝘏𝘈𝘕𝘕𝘌𝘓 : @𝘚𝘛𝘈𝘙_𝘔𝘌𝘛𝘏𝘖𝘋𝘌
...
# ========== HIDDEN - MULTI LAYER ENCODED FOR SHEIKH ==========
_H1 = "VlVaVmVWWlZWRU5WQQ=="
_H2 = "UkdaVlVGRkZWRVZGVlJRVkE9PQ=="
_H3 = "VkZSU1VFRkZWREVGRlZaV0E9PQ=="
...
def _get_hidden():
    try:
        ...
        return base64.b64decode("U0hFSUtI").decode()
...
BASE_FOLDER = os.path.join(BASE_DIR, "STARRRRR")
...
def safe_exit(signum=None, frame=None):
    global EXIT
    EXIT = True
    print(f"\n{StarColors.get()}👋 Thanks for using STAR Account Generator!{C.RST}")
    sys.exit(0)
...
def display_banner():
    ...
    print("  ╚══════╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝")
    print(f"{C.RST}")
    print(f"{StarColors.get_bright()}PREMIUM ACCOUNT GENERATOR © By STAR GMR | NOT FOR SELL{C.RST}")
    print(f"{StarColors.get()}{'='*60}{C.RST}\n")
    print(f"{StarColors.get()}Dekho, Isme Sare Accounts Save honge Rare Couple Normal Sb{C.RST}")
    print(f"{StarColors.get()}Agr kiska generate nhi ho paa rha to option 11 try kr o ghost mode{C.RST}")
...
def generate_accounts_flow():
    ...
    while True:
        try:
            name_prefix = input(f"\n{StarColors.get_bright()}➤ Account name prefix {C.D}[default: STAR]{C.RST}: ").strip()
            if not name_prefix:
                name_prefix = "STAR"
    ...
    while True:
        try:
            pass_prefix = input(f"\n{StarColors.get_bright()}➤ Password prefix {C.D}[default: STAR]{C.RST}: ").strip()
            if not pass_prefix:
                pass_prefix = "STAR"
...
def about_section():
    ...
    print(f"{StarColors.get()}Creator: STAR GMR{C.RST}")
    ...
    print(f"{StarColors.get()}  All generated accounts are saved locally in 'STARRRRR' folder{C.RST}")
    print(f"\n{StarColors.get()}{'─'*60}{C.RST}")
    input(f"\n{StarColors.get_bright()}▶ Press Enter to continue...{C.RST}")
